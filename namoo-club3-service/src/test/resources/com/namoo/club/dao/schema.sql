...
-- SocialPerson
ALTER TABLE SocialPerson
	ADD CONSTRAINT PK_SocialPerson -- SocialPerson 기본키
		PRIMARY KEY (
			email -- 이메일
		);
...
ALTER TABLE CommunityCategory
	ADD CONSTRAINT FK_Community_TO_CommunityCategory -- Community -> CommunityCategory
		FOREIGN KEY (
			cmId -- 커뮤니티아이디
		)
...
-- Club
ALTER TABLE Club
	ADD CONSTRAINT FK_Community_TO_Club -- Community -> Club
		FOREIGN KEY (
			cmId -- 커뮤니티아이디
		)
		REFERENCES Community ( -- Community
...
-- Club
ALTER TABLE Club
	ADD CONSTRAINT FK_CommunityCategory_TO_Club -- CommunityCategory -> Club
		FOREIGN KEY (
			cgId -- 카테고리아이디
		)
		REFERENCES CommunityCategory ( -- CommunityCategory
			cgId -- 카테고리아이디
		);

-- ClubMember
ALTER TABLE Member
	ADD CONSTRAINT FK_SocialPerson_TO_Member -- SocialPerson -> ClubMember
		FOREIGN KEY (
			email -- 이메일
		)
		REFERENCES SocialPerson ( -- SocialPerson
			email -- 이메일
		);

-- ClubMember
ALTER TABLE Member
	ADD CONSTRAINT FK_Club_TO_Member -- Club -> ClubMember
		FOREIGN KEY (
			id -- 아이디
		)
		REFERENCES Club ( -- Club
			clid -- 클럽아이디
		);

-- ClubMember
ALTER TABLE Member
	ADD CONSTRAINT FK_Community_TO_Member -- Community -> ClubMember
		FOREIGN KEY (
			id -- 아이디
		)
		REFERENCES Community ( -- Community
